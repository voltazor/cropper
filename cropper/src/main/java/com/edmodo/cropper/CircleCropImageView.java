...
import com.edmodo.cropper.cropwindow.CircleCropOverlayView;
import com.edmodo.cropper.cropwindow.edge.Edge;
import com.edmodo.cropper.util.ImageViewUtil;

/**
 * Custom view that provides circle cropping capabilities to an image.
 */
public class CircleCropImageView extends FrameLayout {
    ...
    private CircleCropOverlayView mCropOverlayView;
    ...
    private void init(Context context) {
        ...
        final View v = inflater.inflate(R.layout.circle_crop_image_view, this, true);

        mImageView = (ImageView) v.findViewById(R.id.ImageView_image);

        setImageResource(mImageResource);
        mCropOverlayView = (CircleCropOverlayView) v.findViewById(R.id.CropCircleOverlayView);
    }
    ...
    public Bitmap getCroppedImage() {
        ...
        int radius = croppedBitmap.getWidth() / 2;

        Paint paint = new Paint();
        paint.setAntiAlias(true);
        paint.setShader(new BitmapShader(croppedBitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP));

        Bitmap output = Bitmap.createBitmap(croppedBitmap.getWidth(), croppedBitmap.getHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(output);
        canvas.drawRoundRect(new RectF(0, 0, croppedBitmap.getWidth(), croppedBitmap.getHeight()), radius, radius, paint);
